      ...
      <View style={styles.topRow}>
        {isOccupied && tenant_name ? (
          ...
          <View style={styles.vacantIcon}>
            <MaterialCommunityIcons name="home-outline" size={20} color={colors.textMuted} />
          </View>
        )}
        ...
        <View style={styles.nameBlock}>
          ...
          {item.vacating_date && (
            <View style={styles.vacatingChip}>
              <MaterialCommunityIcons name="calendar-remove" size={11} color={colors.warning} />
      ...
      {rent_record ? (
        <View style={styles.breakdown}>
          <View style={styles.colGroup}>
            ...
            <Text style={styles.colValue}>₹{rent_record.base_rent.toLocaleString("en-IN")}</Text>
          </View>
          <View style={styles.colDivider} />
          <View style={styles.colGroup}>
            <Text style={styles.colLabel}>UTILITIES</Text>
            <Text style={[styles.colValue, rent_record.electricity > 0 && { color: colors.blue }]}>
      ...
      {rent_record && rent_record.carry_forward !== 0 && (
        <View style={styles.cfRow}>
          <MaterialCommunityIcons
            name={rent_record.carry_forward > 0 ? "arrow-up-circle-outline" : "arrow-down-circle-outline"}
            size={13}
            color={rent_record.carry_forward > 0 ? colors.danger : colors.success}
          />
          <Text style={[styles.cfText, { color: rent_record.carry_forward > 0 ? colors.danger : colors.success }]}>
      ...
      {isPartial && rent_record && (
        <View style={styles.partialRow}>
          ...
          <Text style={styles.partialDue}>
            Due ₹{(rent_record.total - (rent_record.paid_amount ?? 0)).toLocaleString("en-IN")}
          </Text>
        </View>
      )}
    </TouchableOpacity>
...
const createStyles = (c: AppColors) => StyleSheet.create({
  ...
  vacatingChip: { flexDirection: "row", alignItems: "center", gap: 3, marginTop: 3 },
  ...
  colDivider: { width: 1, backgroundColor: c.borderLight, marginHorizontal: 4 },
  cfRow: {
    flexDirection: "row",
    alignItems: "center",
    gap: 4,
