...
export default function PropertyDetailScreen() {
  ...
  const paidCount = results.filter((r) => r.rent_record?.status === "PAID").length;
  const totalCollected = results.reduce(
    (sum, r) => sum + (r.rent_record?.status === "PAID" ? r.rent_record.total : (r.rent_record?.paid_amount ?? 0)),
    0
  );
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.headerSub}>{property?.address || "Property Overview"}</Text>
        </View>
      ...
      {/* Stats bar — aurora */}
      {!loading && (
        <LinearGradient colors={sBorder} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.statsBorderWrap}>
          <View style={[styles.statsInner, { backgroundColor: sBg }]}>
            <Animated.View style={[StyleSheet.absoluteFillObject, { opacity: sa1 }]}>
              <LinearGradient colors={["transparent", sTeal, "transparent"]} start={{ x: 0, y: 0.35 }} end={{ x: 1, y: 0.65 }} style={StyleSheet.absoluteFillObject} />
            </Animated.View>
            <Animated.View style={[StyleSheet.absoluteFillObject, { opacity: sa2 }]}>
              <LinearGradient colors={[sViolet, "transparent", sViolet]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFillObject} />
            </Animated.View>
            <Animated.View style={[StyleSheet.absoluteFillObject, { opacity: sa3 }]}>
              <LinearGradient colors={["transparent", sMint, "transparent"]} start={{ x: 1, y: 0.2 }} end={{ x: 0, y: 0.8 }} style={StyleSheet.absoluteFillObject} />
            </Animated.View>
            <Animated.View style={[StyleSheet.absoluteFillObject, { opacity: sa4 }]}>
              <LinearGradient colors={[sRose, "transparent"]} start={{ x: 0.5, y: 0 }} end={{ x: 0.5, y: 1 }} style={StyleSheet.absoluteFillObject} />
            </Animated.View>
            <Animated.View style={[StyleSheet.absoluteFillObject, { opacity: sa5 }]}>
              <LinearGradient colors={["transparent", sPink, "transparent"]} start={{ x: 0, y: 1 }} end={{ x: 1, y: 0 }} style={StyleSheet.absoluteFillObject} />
            </Animated.View>
      ...
      <View style={styles.searchBar}>
        <MaterialCommunityIcons name="magnify" size={18} color={colors.inputPlaceholder} style={{ marginRight: 8 }} />
        <TextInput
